...
7、创建init(）方法连接websocket并接受服务器发送过来的数据，实时渲染页面信息。首先是判断浏览器是否支持websocket，若支持则创建websocket实例，则首先通过socket.close()清除之前的socket链接，再将socket置空，接着创建实例socket = new WebSocket(socketUrl);socketUrl为后端通信的路径，在 socket.onopen 中调用心跳机制
